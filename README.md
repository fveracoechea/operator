...
**Project installation, setup, readiness, the crew frontier, Operative dispatch, and question routing work.**
The repository contains the Bun CLI, its local and CI quality gate, `operator install`, `operator setup`, `operator setup readiness`, `operator crew own`, `operator work`, `operator attempt`, `operator question`, and `operator approval`.
Review, cleanup, the live readiness probe, and release automation are not implemented yet.
