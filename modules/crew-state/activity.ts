...
const SETTLED_ASSIGNMENT_STATES = new Set(["registered", "accepted", "invalidated"]);
  ...
  try {
    ...
    const rows = sqlite
      .query(
        `select a.id as assignment_id, a.state as state, t.id as attempt_id, t.state as attempt_state
         ...
         where a.state not in ('registered', 'accepted', 'invalidated') or t.id is not null`,
      )
    ...
    return {
      ...
      active: rows.map((row) => ({
        ...
        attemptState: row.attempt_state === null ? null : String(row.attempt_state),
      })),
    };
  } catch (error) {
...
export { SETTLED_ASSIGNMENT_STATES };
